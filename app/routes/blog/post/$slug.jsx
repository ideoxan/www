...
export default function BlogPost() {
    ...
    let share = {
        ...
        twitter: `https://twitter.com/intent/tweet?text=${ encodeURIComponent(post.attributes.title + " on @ideoxan\n" + url) }`,
        linkedin: `https://www.linkedin.com/shareArticle?mini=true&url=${ encodedURL }&title=${ encodeURIComponent(post.attributes.title) }&summary=&source=`,
    }
                    ...
                    <article className="flex flex-col max-w-3xl w-full p-8">
                        <div className="flex flex-col md:flex-row">
                            <div className="flex flex-row">
                                <img src="/images/ix_logo_purple_1024_1024.png" alt={post.attributes.author + "'s Profile Picture"} className="w-8 h-8 my-auto rounded-full" />
                                <div className="flex flex-col ml-4">
                                    <p className="font-sans font-medium text-left text-sm text-gray-50 opacity-80">Posted by <a href={post.attributes.author.replace("@", "/user/")} className="hover:underline">{post.attributes.author}</a></p>
                                    <p className="mt-0.5 font-sans font-bold text-left text-2xs text-gray-100 opacity-80">{formatDate(post.attributes.date)}</p>
                                </div>
                            </div>
                            <div className="flex flex-row mt-6 md:ml-auto space-x-6 md:my-auto">
                                <button
                                    ...
                                    onClick={() => { window.open(share.fb, "_blank") }}
                                >
                                    <Facebook width={16} />
                                </button>
                                <button
                                    className="text-gray-50 opacity-50 hover:opacity-100 flex flex-row"
                                    onClick={() => { window.open(share.twitter, "_blank") }}
                                >
                                    <Twitter width={16} />
                                </button>
                                <button
                                    className="text-gray-50 opacity-50 hover:opacity-100 flex flex-row"
                                    onClick={() => { window.open(share.linkedin, "_blank") }}
                                >
                                    <Linkedin width={16} />
                                </button>
                            </div>
                        </div>
                        <h1 className="mt-8 pb-2 font-sans font-extrabold tracking-tight text-2xl sm:text-3xl md:text-4xl glow-text-white">{post.attributes.title}</h1>
                        <h2 className="italic mt-2 font-sans font-semibold tracking-tight text-sm sm:text-base text-gray-100">{post.attributes.description}</h2>
                        <img src={post.attributes.thumbnail} className="mt-6 md:mt-8 w-full h-auto object-cover rounded-md" alt={post.attributes.title + "'s thumbnail"} />
                        <div className="mt-6 md:mt-8 blog-content"><PostMDX /></div>
                    </article>
